...
module cone_head_bolt_my (length=10, thread_d=3.9, head_d=7.5){
	
	//echo("7.35/3.30=",7.35/3.0);	= 2.5
	//echo("9.15/3.50=",9.15/3.50); = 2.6
	head_h = head_d/2.5;
	tolerance=0.0;
	quality=24;
	allen_o = 0;
	a_depth= 0;
	//(length, thread_d, head_h, head_d, a_depth, tolerance, quality, allen_o, thread, pitch)
	//translate ([30, -20, 0]) cone_head_bolt (10, 3, 1.5, 5.5, 2, 0.2, 32);
	cone_head_bolt(length,thread_d, head_h, head_d, a_depth, tolerance, quality, allen_o);
	//cylinder(10,r=thread_d/2,$fn=32,center=true);
	//  cone_head_bolt(length,thread_d, head_h, head_d,   6,         0.0,           32,       2);
	//translate([0,0,length]) cylinder(head_h,r=head_d/2,$fn=30);
}

module hex_bolt_my(length, thread_d, head_h, head_d){
	//hex_bolt (length, thread_d, head_h, 3/8, 1/128, 32, 0, "imperial", 28);
  //hex_bolt (length, thread_d, head_h, head_d, tolerance, quality, bool_cut, thread, pitch) {
	
	if (length<0)
		translate([0,0,length-head_h])
			hex_bolt (abs(length), thread_d, head_h, head_d, 0, 			32, 	0, 		"metric", 0);
	else 
		hex_bolt (abs(length), thread_d, head_h, head_d, 0, 			32, 	0, 		"metric", 0);
	//cylinder(10,r=head_d/2,$fn=32);
}


module cone_head_bolt_m7 (length=10){
	...
	if (length<0)
		translate([0,0,length-head_h])
			cone_head_bolt_my(abs(length),thread_d, head_d);
	else
		cone_head_bolt_my(abs(length),thread_d, head_d);
}


module hex_bolt_m7(length=10, tolerance=0){
	//translate ([10, -20, 0]) hex_bolt (3/8, 2/8, 1/8, 3/8, 1/128, 32, 0, "imperial", 28);
	thread_d=3.90+0.2+tolerance;
	head_h	=3.12+0.5+tolerance;
	head_d	=6.82+0.18+tolerance;
	hex_bolt_my(length,thread_d,head_h,head_d);
}

module hex_bolt_m8(length=10, tolerance=0.1){
	//translate ([10, -20, 0]) hex_bolt (3/8, 2/8, 1/8, 3/8, 1/128, 32, 0, "imperial", 28);
	thread_d=4.69+tolerance;
	head_h	=3.31+tolerance;
	head_d	=7.82+tolerance;
	hex_bolt_my(length,thread_d,head_h,head_d);
}
...
//  hex_bolt (length, thread_d, head_h, head_d, tolerance, quality, thread, pitch);
    ...
    translate ([10, -20, 0]) hex_bolt (3/8, 2/8, 1/8, 3/8, 1/128, 32, 0, "imperial", 28);
    //#translate ([10, -30, 0]) hex_bolt_my(10,2,5,5);
    translate ([10, -30, 0]) hex_bolt_m7(10);
    translate ([10, -40, 0]) hex_bolt_m8(10);
...
//  cone_head_bolt (length, thread_d, head_h, head_d, a_depth, tolerance, quality, allen_o, thread, pitch);
    ...
    translate ([30, -20, 0]) cone_head_bolt (10, 3, 1.5, 5.5, 2, 0.2, 32);
   // translate ([30, -30, 0]) my_cone_head_bolt();
    translate ([30, -30, 0]) cone_head_bolt_m7();
